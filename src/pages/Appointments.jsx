                        ...
                        appointments.map(appt => (
                            <div key={appt._id} className={`bg-white p-5 rounded-3xl shadow-sm border transition-all duration-300 ${appt.reminderEnabled ? 'border-emerald-200 ring-1 ring-emerald-100 shadow-emerald-50' : 'border-gray-100'}`}>
                                ...
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <span className={`text-[10px] font-extrabold px-3 py-1.5 rounded-full uppercase tracking-wide border ${appt.status === 'CONFIRMED' ? 'bg-green-100 text-green-700 border-green-200' :
                                            appt.status === 'CHECKED_IN' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' :
                                                appt.status === 'IN_PROGRESS' ? 'bg-purple-100 text-purple-700 border-purple-200' :
                                                    appt.status === 'COMPLETED' ? 'bg-purple-100 text-purple-700 border-purple-200' :
                                ...
                                <div className="bg-gray-50/80 p-4 rounded-2xl border border-gray-100 mb-5 space-y-3">
                                    ...
                                    {appt.status === 'COMPLETED' && (
                                        <div className="border-t border-gray-100 mt-4 pt-4 flex justify-end">
                                            <button
                                                onClick={() => navigate('/patient-appointments', { state: { prefill: appt } })}
