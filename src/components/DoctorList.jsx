  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      const appointmentData = {
        ...
        hospitalId: bookingHospital.id.startsWith('db-') ? bookingHospital.id.substring(3) : null,
        doctor: doctorName,
        doctorId: selectedDoctorId || null,
        doctorEmail: selectedDoctorEmail || null,
        ...
        <div className="fixed inset-0 z-[1000] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-sm rounded-3xl p-5 shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 pr-1 custom-scrollbar -mr-1">
              {step === 'form' && (
                <form onSubmit={handleBooking} className="space-y-4 pb-2">
                  ...
                  <div className="bg-emerald-50 p-3 rounded-2xl border border-emerald-100">
                    <p className="text-sm font-bold text-emerald-900 leading-tight">{bookingHospital.name}</p>
                        ...
                        {bookingHospital.doctors.map((doc, i) => (
                          <button
                            ...
                            onClick={() => {
                              ...
                              setSelectedDoctorId(doc.userId || doc._id || '');
                              setSelectedDoctorEmail(doc.email || '');
                            }}
                            className={`w-full flex items-center gap-3 p-2.5 rounded-xl border-2 transition-all text-left ${selectedDoctor === doc.name
                              ? 'border-emerald-500 bg-emerald-50 ring-1 ring-emerald-100'
                              : 'border-slate-100 bg-white hover:border-slate-200 shadow-sm'
                              }`}
                          >
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${selectedDoctor === doc.name ? 'bg-emerald-500 text-white' : 'bg-slate-100 text-slate-600'
                              }`}>
                              {doc.name?.charAt(0)?.toUpperCase() || 'D'}
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="font-bold text-xs text-slate-800 truncate">{doc.name}</p>
                              <p className="text-[10px] text-slate-500">{doc.specialty || t.opd}</p>
                            </div>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder={lang === 'en' ? 'Optional' : 'ఐచ్ఛికం'} className="w-full bg-slate-50 border border-slate-100 p-3 rounded-xl font-bold text-sm text-slate-800 outline-none focus:ring-2 focus:ring-emerald-500 transition" value={reason} onChange={e => setReason(e.target.value)} />
                  </div>

                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1">{t.date}</label>
                      <input required type="date" min={new Date().toISOString().split('T')[0]} className="w-full bg-slate-50 border border-slate-100 p-3 rounded-xl font-bold text-sm text-slate-800 outline-none focus:ring-2 focus:ring-emerald-500 transition" value={bookDate} onChange={e => setBookDate(e.target.value)} />
                    </div>
                    <div>
                  ...
                  <button type="submit" className="w-full bg-emerald-600 text-white py-3.5 rounded-xl font-bold shadow-lg shadow-emerald-100 mt-2 hover:bg-emerald-700 transition active:scale-95">
                    {t.confirm}
                  </button>
                </form>
              )}
